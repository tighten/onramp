<template>
    <transition name="slide">
        <div
            v-if="isVisible"
            class="px-6 text-center border-t-2 border-b-2 border-gray-200 bg-white"
            role="alert"
        >
...
export default {
    ...
    mounted() {
        document.addEventListener('DOMContentLoaded', () => {
            if (localStorage.showTrackBanner) {
                this.isVisible = JSON.parse(localStorage.showTrackBanner)
    ...
    watch: {
        isVisible(value) {
            localStorage.showTrackBanner = value;
        },
    }
}
</script>
