...
export default function useModules(props) {
    ...
    const userModuleIds = computed(
        () => new Set((props.userModules || []).map((id) => parseInt(id, 10)))
    );
    const completedModuleIds = computed(
        () => new Set((props.completedModules || []).map((id) => parseInt(id, 10)))
    );
    const completedResourceIds = computed(
        () => new Set((props.userResourceCompletions || []).map((id) => parseInt(id, 10)))
    );
    ...
    const filterStandardModules = (skillLevel) => {
        ...
        return showAllModules.value
            ? [
                  ...
                  ...modulesByLevel.filter((mod) => !filterByUser(mod)),
              ]
            : modulesByLevel.filter(filterByUser);
    };

    const beginnerModules = computed(() => filterStandardModules('Beginner'));
    const intermediateModules = computed(() => filterStandardModules('Intermediate'));
    ...
    const currentBonusModules = computed(() => {
        ...
        return showAllModules.value
            ? [
                  ...
                  ...props.bonusModules.filter((mod) => !filterByUser(mod)),
              ]
            : props.bonusModules.filter(filterByUser);
    });
