    ...
    <div class="w-full bg-off-white">
        ...
        <x-hero id="top">
            ...
            @if (auth()->check() && auth()->user()->hasTrack())
                <div
                    class="mt-6 text-sm text-comet">{!! __('You are on the :track track.', ['track' => '<span class="font-semibold">'.auth()->user()->track->name.'</span>']) !!}
                    <a class="ml-2 mr-4 font-semibold text-teal"
                       href="{{ url_wlocale('preferences') }}">{{ __('Change settings') }}</a>
                </div>
            @endif
        </x-hero>
