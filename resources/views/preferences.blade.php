	...
	<x-panel>
		<form method="post" action="{{ route_wlocale('user.preferences.update') }}">
			...
			<div>
				...
				<div class="mt-5">
					<label for="{{ $localePreferenceKey }}" id="locale-label" class="block text-sm leading-5 text-steel md:text-base">
					...
					<div class="p-5 mt-6 border rounded-md border-silver md:p-8">
						...
						<div class="flex items-center mt-4 first:mt-0">
							<input id="lang_pref_{{ $index }}" name="{{ $resourceLanguagePreferenceKey }}" value="{{ $index }}" type="radio" {{ $currentResourceLanguagePreference==$index ? 'checked' : '' }} class="flex-none w-4 h-4 transition duration-200 ease-in-out text-purple" />

							<label for="lang_pref_{{ $index }}" class="ml-3">
								<span class="block text-sm font-medium leading-5 text-steel md:text-base">{{ $label }}</span>
							</label>
						</div>
						@endforeach
					</div>
				...
				<div class="mt-5">
					...
					<div class="flex flex-wrap px-5 pb-2 mt-6 border rounded-md border-silver lg:flex-no-wrap md:px-8">
						<div class="flex-auto w-full my-5 lg:flex-even md:my-8">
							<label for="{{ $localePreferenceKey }}" id="locale-label" class="text-base font-medium text-steel">
								{{ __('Preferred Language') }}
							</label>

							<div class="relative max-w-xs mt-4">
								<select class="block w-full px-4 py-2 pr-8 text-sm leading-tight bg-white border rounded-md border-gray shadow appearance-none hover:border-gray focus:outline-none focus:shadow-outline{{ $errors->has('name') ? ' border-cabernet' : '' }}" name="{{ $localePreferenceKey }}" aria-labelledby="locale-label">
									@foreach (Facades\App\Localization\Locale::slugs() as $slug)
									<option value="{{ $slug }}" @if (old($localePreferenceKey)==$slug || $preferredLocale==$slug) selected @endif>
										{{ Facades\App\Localization\Locale::languageForLocale($slug) }}
									</option>
									@endforeach
								</select>

								<div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-steel">
				...
				<div class="pb-5 mt-12">
					<h2 class="text-xl font-medium md:text-2xl lg:text-3xl">{{ __('Resource Digest Preferences') }}</h2>
				</div>
				...
				<div class="mt-5">
					...
					<div class="flex flex-wrap p-5 mt-6 border rounded-md border-silver md:p-8">
						<div class="flex-auto w-full lg:flex-even">
							...
							<div class="relative max-w-xs mt-4">
								<select class="block w-full px-4 py-2 text-sm pr-8 leading-tight bg-white border rounded-md border-gray shadow appearance-none hover:border-gray focus:outline-none focus:shadow-outline{{ $errors->has('track') ? ' border-cabernet' : '' }}" name="track" aria-labelledby="track-label">
									@foreach (App\Models\Track::all() as $track)
									<option value="{{ $track->id }}" {{ auth()->user()->track_id == $track->id || old('track') == $track->id ? 'selected' : '' }}>
										{{ $track->name }}</option>
									@endforeach
								</select>
